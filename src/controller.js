...
function getPlaces(req, res, next) {
  request(BASE_URL + 'location/bbox?bbox=-124.848974,24.396308,-66.885444,49.384358&limit=100&skip=0',
...
function rangePlaces(req, res, next) {
  request(BASE_URL + `search?from=${req.query.lower}/1/1&to=${req.query.upper}/1/1`,
  function (error, response, body) {
    if (!error && response.statusCode == 200) {
      res.send(body)
    }
